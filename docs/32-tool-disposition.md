...
| `vecobj.py` | `tools/build_assets.py` | THE DISPOSITION'S REASON WAS WRONG and the decision was right. It said 'output committed as src/gen/P1VECT.INC'; that file names emit_pascal_data2.py, and vecobj.py never wrote it -- it writes into assets/. Checked instead: build_assets.py carries its own EMBEDDED table producing the same vector_globe and vector_logo_a, assets/ is committed (163 files), and assets/README.md names build_assets.py as the producer with the DGROUP offsets in its table. So it is spent, superseded by a tool that STAYS with the record. **CORRECTED AGAIN, 1 Sep 2026, and this time the DECISION was half wrong.** The check above verified the part 001 outputs -- vector_globe and vector_logo_a -- and stopped there. vecobj.py also produced part 002's four solid models and five projections of each, and build_assets.py's EMBEDDED table produced NONE of those: 24 files lost their producer the day this row was written. They were not noticed as missing because they were still on disk, so --audit could only class them as unexplained extras, and a KEPT row went in calling them "rendered by hand". Nothing regenerated them for six months, so nothing compared them either, and every one of the 20 projections was drawn with the Enterprise's index bias applied to all four models -- wrong for three of them. carve_solid_models() in build_assets.py generates them now, with the bias per model and an out-of-range index refusing instead of being filtered. **The lesson for a disposition: 'superseded by X' has to name EVERY output, and the measurement is regenerating them all and diffing, not confirming that the successor produces some of them.** A tool's outputs sitting committed in the tree is what lets a partial supersession look complete |
