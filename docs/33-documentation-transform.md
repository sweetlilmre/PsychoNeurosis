...
5. **`tagcheck.py` clean on the batch** — 0 problems in the stripped copy. It reports three kinds: an untagged paragraph carrying apparatus, an INERT tag (one not at a paragraph start, which does nothing and ships the marker), and a CUT SENTENCE (a trim that took a citation from the middle of a sentence rather than the front of one, leaving the prose hanging). The third is the one nothing else in this gate can see — no tell survives for the leak check, and `clean.py`'s self-check compares code, not comments.
