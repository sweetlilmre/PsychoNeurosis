...
```
  P1LOGO               transcribed   1012:0311      LoadLogo
  P1LOGO               transcribed   1012:0366      BuildCell
  P1TITLE               transcribed   1082:0000      Load
  P1TITLE               transcribed   1082:01d0      DrawRevealed
  P1TITLE               stub          1082:0200      DrawChar
  P1MOSAIC               transcribed   10e3:0206      MosaicRun
  P1MOSAIC               transcribed   10e3:0206      Scene3
  P1BALLS               transcribed   12b2:00c0      StarStep
  P1BALLS               transcribed   12b2:00f1      StarDraw
  P1BALLS               transcribed   1107:0245      DrawBlobs
  P1BALLS               transcribed   1107:0562      FrameStep
  P1BALLS               transcribed   1107:076e      BannerRun
  P1VECTOR               transcribed   12c5:088b      LoadObject
  P1VECTOR               transcribed   12c5:052a      DrawPoint
  P1VECTOR               transcribed   12c5:061d      RenderObject
  P2SOLID               transcribed   108b:0b42      DrawStars
...
- **8** of 34 units carry any tag at all: `P1LOGO`, `P1TITLE`, `P1MOSAIC`, `P1BALLS`, `P1VECTOR`, `P2SOLID`, `P3BLOCKS`, `P3MORPH`.
...
| 001 | P1LOGO–P1VECTOR, P1INTRO | 56 | **15** |
| 002 | P2GARAGE, P2SOLID, P2VIEW, P2MAIN (+MODEX, FIXMATH shared) | 52+8 | **1** (`P2SOLID.DrawStars`) |
| 003 | P3TUNNEL/STARS/MORPH/GLOBE/BLOCKS/WAVES/SPRITES, P3MAIN | 63 | **2** |
| 004 | P4LEMS | 32 | **0** |
| 005 | P5MESH–P5PATCH, P5MAIN | 23 | **0** |
| 006 | P6WHOOSH–P6CREDIT, P6MAIN | 29 | **0** |
| 007 | P7FLIC, P7MAIN | 16 | **0** |
...
1. **3 of the 18 rows are spurious.** `MARK`'s note group is `(.*?)` under DOTALL and may cross brace boundaries, so the *legend comment* in each of the P1LOGO/P1TITLE/P1MOSAIC file headers ("Markers: `[transcribed]` read out of the binary, `[inferred]` implied by its call sites, `[stub]` signature only") matches, and is attributed to whatever routine happens to be declared next. Captured note lengths: **1030, 2427 and 1909 characters**. The address is then scavenged from unrelated intervening text.
   - `P1LOGO LoadLogo 1012:0311` is wrong — `1012:0311` is `P1LOGO.SaveUnder`'s address per `asmverify`'s own marker.
   - `P1TITLE Load 1082:0000` and `P1MOSAIC MosaicRun 10e3:0206` come from the same over-capture. `10e3:0206` is then reported for *two* rows (`MosaicRun` and `Scene3`), which is the correct address for only one.
2. **The one remaining `[stub]` is stale.** `P1TITLE.DrawChar` is marked "signature only", but `src/P1TITLE.PAS:171` is a fully implemented two-loop glyph blitter with a 25-line derivation comment above it. So the headline "1 stub" is a false positive, and "0 inferred / 0 stubs / 0 empty bodies" is the true state of the tagged subset.
...
| stable identity | weak — `name` only, unqualified; `P4LEMS.Blit` and `P6TILES.Blit`, and four separate `SetPalette768`/`GetPalette768`, collide on name |
...
Where the two overlap they **disagree**: `asmverify` puts `1012:0311` at `P1LOGO.SaveUnder`; the ledger prints it against `LoadLogo`. `asmverify` is right (its marker is machine-parsed and address-checked against the binary).
...
Two of those statements are now out of date: 005–007 **do** have harnesses and scene splits (`P5MESH`–`P5PATCH`, `P6WHOOSH`–`P6CREDIT`, `P7FLIC` with `TP5S*`, `TP6S*`, `TP7S1` built in `run/`), and the stubs/inferred/empty bodies it lists there are gone — the ledger, `asmverify` and a direct search all report zero. So the doc's "part 003 is audited, the rest are not" framing predates the 005–007 work now in the tree.
