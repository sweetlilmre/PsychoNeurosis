...
| **A hand edit to a GENERATED file** | **Found 1 Sep 2026, after this document was written, and it is the one hazard here that outlived the effort.** Ticket [#70](https://github.com/sweetlilmre/PsychoNeurosis/issues/70) stripped apparatus from all seven `src/gen/*.INC` files by editing them directly. They are generated, and `emit_pascal_data2.py` was not given the same edit — so running the emitter reverted a closed ticket, in seven files at once, and the only thing preventing it was nobody happening to run it. **Every instrument in the tree was blind to this by construction**: the divergence was comment-only and the data identical in all seven, so no compiled byte moved, the build stayed byte-identical and all ten artefact rows went on holding. Nothing was wrong with the tree; what was wrong is that two things claimed to be the same file and nothing compared them. Now `emit_pascal_data2.py --check`, which regenerates into a temporary directory and classifies each difference as comments-only or data. The general form: **a `[re]` tag or a trim applied to a generated file has to be applied to its generator, and the transform's gates cannot see the difference** — a stripped copy is checked against `src`, never `src` against what produced it. |
