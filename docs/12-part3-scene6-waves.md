...
**A build-fidelity fact that fell out of the byte comparison**: the `_fpu` original variants are a post-build PATCH, not a separate compilation — `NEUROSIS_003.exe` and `NEUROSIS_003_fpu.exe` are the same size and differ exactly by TP7's `CD 3x` emulator interrupts rewritten in place to raw `9B`+ESC x87 opcodes. The reconstruction therefore keeps TP7's `$E+` default and treats the fpu patching as out of scope, like LZEXE.
