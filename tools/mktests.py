...
Names are 8.3 and uniform: part number, scene number. build.toml's `identity`
glob registers anything matching test/TP*.PAS automatically, so adding a row
here is the only step.

    python tools/mktests.py                                  write them all
    kit/tools/pascal/build.py build.toml TP3S1               build one
    kit/tools/pascal/build.py build.toml                     build everything
...
SCENES = [
    ...
    ("TP3S1", "P3Tunnel",  "Scene1", VGA_USES, VGA_OPEN, VGA_CLOSE,
     "part 003 scene 1 -- the tunnel"),
    ("TP3S2", "P3Stars",   "Scene2", VGA_USES, VGA_OPEN, VGA_CLOSE,
     "part 003 scene 2 -- the star tube"),
    ("TP3S3", "P3Morph",   "Scene3", VGA_USES, VGA_OPEN, VGA_CLOSE,
     "part 003 scene 3 -- the morph"),
    ("TP3S4", "P3Globe",   "Scene4", VGA_USES, VGA_OPEN, VGA_CLOSE,
     "part 003 scene 4 -- the globe"),
    ("TP3S5", "P3Blocks",  "Scene5", VGA_USES, VGA_OPEN, VGA_CLOSE,
     "part 003 scene 5 -- the blocks"),
    ("TP3S6", "P3Waves",   "Scene6", VGA_USES, VGA_OPEN, VGA_CLOSE,
     "part 003 scene 6 -- the waves"),
    ("TP3S7", "P3Sprite", "Scene7", VGA_USES, VGA_OPEN, VGA_CLOSE,
...
EXIT_PROC = """{ Runs on the way out of Halt, of a run-time error, or of a
...
HEAD = """{ ===========================================================================
  ...
  Build:  kit/tools/pascal/build.py build.toml %(prog)s
  =========================================================================== }
...
%(exit)s%(sayproc)s{ F IS A LOCAL, for the same reason RestoreTextMode ends the
