...
- **The Bash tool collapses `\\` into `\` before bash ever sees the command**, and nothing else — `\'`, `\"`, `\t`, `\n`, `\$` pass through. Proven with `cat` in a quoted heredoc, which interprets nothing, yet turned `A\\B` into `A\B`. Two failures follow from this one bug and look unrelated: generated scripts get broken string literals (`'\\n'` arrives as `'\n'`; hence the endless `SyntaxWarning: invalid escape sequence '\s'`), and a `\\` next to a quote collapses to `\` + quote, which bash reads as an *escaped* quote — unbalancing the command and reporting `unexpected EOF while looking for matching ''` at the wrong line. **So: never put a backslash in a Bash command. Write the script with the Write tool and run it**, build backslashes with `chr(92)`, and pass prose via `--body-file` rather than inline.
