...
| **`src` into `src-clean`, the documentation transform** | [The map](https://github.com/sweetlilmre/PsychoNeurosis/issues/52), then [`docs/34-documentation-transform-run.md`](docs/34-documentation-transform-run.md) | **COMPLETE and CLOSED, 20 of 20 tickets, 1 Sep 2026.** `src-clean` is a derived, regenerable documentation copy of the whole tree: tree-wide `tagcheck` 0 across all 84 files, it builds on its own, and **all ten artefacts hold at R7 from that build**. It is NEVER hand-edited — regenerate it with `clean.py src src-clean --exclude asm/shared-exempt.txt`. `docs/34` is the record: eleven expired premises, the hazard table with real hit counts, and what the next target should inherit |
...
## The state of this working tree, as of 1 Sep 2026

**Everything is committed and pushed on `main`, there are NO open tickets, and the plan has no open investigations** — 111 of 111 resolved, 189 rows. Check both rather than trusting this line, because it is the one that goes stale first:

    gh issue list --repo sweetlilmre/PsychoNeurosis --state open
    .venv/Scripts/python.exe kit/tools/pascal/plan.py status.toml --report

An earlier version of this sentence named `vangelistracker-build` as the branch, the kit map as the one open ticket, and three remaining investigations including pacing as the main line. Every one of those was true when written. **So there is no standing queue of work in this repository right now**, and what is left is in `docs/continuation.md`'s "What is actually left" — open questions rather than tickets.

The remote is private `sweetlilmre/PsychoNeurosis`, default branch `main`. **`main` is current, clean and pushed, and it is where the work happens** — it was fast-forwarded 451 commits from `vangelistracker-build` on 28 Aug 2026 and both then pointed at the same commit. An earlier version of this line said `main` was behind and everything lived on the branch; that was true when written. History contains third-party bytes — `bin/NEUROSIS.008` and `work/split/NEUROSIS_008.exe` are DemoVT — acceptable only while the repo stays private, and a blocker on ever making it public. A lesson kept from an earlier version of this section: treat a "not mine" claim about tree contents as needing a check against the diff, not as fact.
